...
const DungeonParties = require("../../models/DungeonParties");
...
// Load dungeons safely
let dungeons = [];
try {
  const dungeonsData = JSON.parse(
    fs.readFileSync(path.join(__dirname, "../../config/dungeons.json"), "utf8")
  );
  dungeons = Array.isArray(dungeonsData) ? dungeonsData : dungeonsData.dungeons || [];
} catch (error) {
  console.error("Failed to load dungeons.json:", error);
  dungeons = ["RIN: IZCORGIKY"]; // Fallback
}

async function PartyDungeonFinder(interaction) {
 const dungeonName = interaction.options.getString("dungeon");
      const date = interaction.options.getString("date");
      const time = interaction.options.getString("time");
      const utc = interaction.options.getString("utc");

      const { embed: validationEmbed, unix } = await validateDungeonInput(
        date,
        time,
        utc
      );
      ...
      let dungeonSettings;
      try {
        dungeonSettings = await GuildModule.findOne({
          guildId: interaction.guildId,
          moduleId: "party_dungeon",
        });
        if (!dungeonSettings) {
          const embed = new EmbedBuilder()
            .setTitle("Settings Error")
            .setDescription(
              "Could not find Dungeon Party Finder settings, please contact the administrator."
            )
            .setColor("Red");
        ...
        const embed = new EmbedBuilder()
          .setTitle("Settings Error")
          .setDescription(
            "Could not fetch Dungeon Party Finder settings, please contact the administrator."
          )
          .setColor("Red");
      ...
      const embed = new EmbedBuilder()
        .setColor(0x5865f2)
        .setTitle(`⚔️ Dungeon Party Finder - ${dungeonName}`)
        .setDescription("Click a role button below to join the party!")
        .addFields(
          { name: "🗓️ Date & Time", value: whenField, inline: false },
          { name: "🛡️ Tanks (0/1)", value: "No one yet", inline: false },
          { name: "💚 Healers (0/1)", value: "No one yet", inline: false },
          { name: "⚔️ DPS (0/3)", value: "No one yet", inline: false }
        )
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("dungeon_role_tank")
          .setLabel("TANK")
          .setEmoji("🛡️")
          .setStyle(ButtonStyle.Primary),
        new ButtonBuilder()
          .setCustomId("dungeon_role_healer")
          .setLabel("HEALER")
          .setEmoji("💚")
          .setStyle(ButtonStyle.Success),
        new ButtonBuilder()
          .setCustomId("dungeon_role_dps")
          .setLabel("DPS")
          .setEmoji("⚔️")
          .setStyle(ButtonStyle.Danger),
        new ButtonBuilder()
          .setCustomId("dungeon_leave_party")
      ...
      let content;
      if (dungeonSettings.enabled) {
        content = `<@&${dungeonSettings.settings.roleId}>`;
      } else {
        content = "";
      }
      ...
      // Store party data
      const party = await DungeonParties.create({
        guildId: interaction.guildId,
        dungeonName,
        tanks: [],
        healers: [],
  ...
  async function autocompleteDungeon(interaction) {
    try {
      ...
      if (optionName === "dungeon") {
        // ✅ Show dungeons list
        choices = dungeons.map((dungeon) => ({ name: dungeon, value: dungeon }));
    ...
    async function handleButtonDungeon(interaction) {
    // Find which party this button belongs to
    const channel = interaction.channel;
    const messages = await channel.messages.fetch({ limit: 50 });

    for (const [msgId, msg] of messages) {
      const party = await DungeonParties.findOne({ messageId: msgId });
      if (party) {
...
async function handleInteraction(interaction, partyMessageId) {
  ...
  const party = await DungeonParties.findOne({ messageId: partyMessageId });
  if (!party) {
    const partyEmbedError = new EmbedBuilder()
      .setTitle("Party Finder")
...
async function showClassSelection(interaction, role, partyMessageId) {
  ...
  const buttons = classes[role].map((className) => {
    ...
    return new ButtonBuilder()
      .setCustomId(`dungeon_class_${role}_${className.replace(" ", "")}`)
      .setLabel(className)
      .setStyle(ButtonStyle.Secondary)
...
async function showSpecSelection(interaction, role, className, partyMessageId) {
  ...
  const specButtons = specs[classKey].map((spec) => {
    ...
    return new ButtonBuilder()
      .setCustomId(`dungeon_spec_${role}_${classKey}_${spec}`)
      .setLabel(spec)
      .setStyle(ButtonStyle.Primary)
      .setEmoji(emoji); // assign emoji here
  });
  ...
  const backButton = new ButtonBuilder()
    .setCustomId(`dungeon_back_class_${role}`)
    .setLabel("Back")
    .setStyle(ButtonStyle.Secondary);
...
async function addToParty(interaction, role, className, spec, partyMessageId) {
  ...
  const maxSlots = { tanks: 1, healers: 1, dps: 3 };
  ...
  if (removedFromRole && party.bench[removedFromRole].length > 0) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `🎉 You’ve been promoted from the bench to the main party as a **${nextMember.class} - ${nextMember.spec} (${nextMember.range})** for **${party.dungeonName}**!`
        )
        .setColor("Green");
...
async function validateDungeonInput(date, time, utc) {
  ...
  if (
    ...
    return {
      embed: new EmbedBuilder()
        ...
        .setDescription(
          "To schedule a dungeon, you must provide **date**, **time**, and **UTC offset** together.\n\nExample:\n`/party dungeon dungeon:<name> date:15/10 time:18:30 utc:UTC+2`"
        ),
    };
  }
...
module.exports = {PartyDungeonFinder, autocompleteDungeon, handleButtonDungeon};