...
export default function DungeonSettings({ guildId, user }) {
  ...
  const fetchSettings = async () => {
    try {
      const response = await axios.get(
        `/guilds/${guildId}/modules/party_dungeon`,
        {
      ...
      console.error("Failed to fetch Dungeon Party Finder settings:", err);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      await axios.put(
        `/guilds/${guildId}/modules/party_dungeon/settings/${user.id}/@${user.username}`,
        {
          settings,
        },
        { withCredentials: true }
  ...
  return (
    <form onSubmit={handleSave} className={styles.settingsForm}>
      ...
      <div className={styles.formGroup}>
        <label htmlFor="deleteAfter">Hours to delete after dungeon *</label>
        <input
          id="deleteAfter"
          className={styles.select}
          name="deleteAfter"
        ...
        />
        <small>
          How many hours after creation or start of the dungeon should it be
          deleted?
        </small>
      </div>
